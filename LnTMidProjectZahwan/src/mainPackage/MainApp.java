...
public class MainApp {
	...
	public void addEmployee() {
        ...
        // Count employees with the same title
        int count = 0;
        ...
        // Calculate bonus for earlier employees with the same title
        double bonusPercentage = 0.0;
        if (title.equals("Admin")) {
            bonusPercentage = (count > 1) ? 0.05 : 0;
        } else if (title.equals("Supervisor")) {
            bonusPercentage = (count > 1) ? 0.075 : 0;
        } else if (title.equals("Manager")) {
            bonusPercentage = (count > 1) ? 0.10 : 0;
        }

        double bonus = 0.0;
        for (Employee emp : employees) {
            if (emp.getTitle().equals(title) && emp != employee) {
                bonus += emp.getSalary() * bonusPercentage;
                emp.setSalary(emp.getSalary() + bonus);
            }
        }

        if (count > 1) {
        	System.out.println("Bonus of " + (bonusPercentage * 100) + "% added to employees with title: " + title + ". Employees affected: ");
            for (Employee emp : employees) {
                if (emp.getTitle().equals(title) && emp.getSalary() != salary) {
